...
public class Main {
    ...
    @Deprecated
    public static double _compute(String expression, double value, HashMap<String, Double> map) throws MathParser.SyntaxParseException {
    ...
    public static double compute(String expression, double value, HashMap<String, Double> map) throws MathParser.SyntaxParseException {
        ...
        List<String> coefficientsOfThis = getCoefficients(expression);

        if (!map.isEmpty()) {
            for (Map.Entry<String, Double> entry : map.entrySet()) {
                String coefficient = entry.getKey();
                double v = entry.getValue();
                if (containsAnyOf(map, coefficientsOfThis))
                    expression = replaceCoefficient(expression, coefficient, v);
            }
        }
